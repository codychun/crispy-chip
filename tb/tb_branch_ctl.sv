...
module tb_branch_ctl();
    ...
    initial begin
        ...
        #10;
        assert(pc_sel == 2'b01) else $error("BEQ Taken: pc_sel = %b (Expected: 01)", pc_sel);
        $display("BEQ Taken:     pc_sel = %b (Expected: 01)", pc_sel);

        // BEQ: Not Taken (zero = 0) -> Expect pc_sel = 2'b00
        zero = 1'b0;
        #10;
        assert(pc_sel == 2'b00) else $error("BEQ Not Taken: pc_sel = %b (Expected: 00)", pc_sel);
        ...
        less_than = 1'b1;
        #10;
        assert(pc_sel == 2'b01) else $error("BLT Taken: pc_sel = %b (Expected: 01)", pc_sel);
        $display("BLT Taken:     pc_sel = %b (Expected: 01)", pc_sel);

        // BLT: Not Taken (less_than = 0) -> Expect pc_sel = 2'b00
        less_than = 1'b0;
        #10;
        assert(pc_sel == 2'b00) else $error("BLT Not Taken: pc_sel = %b (Expected: 00)", pc_sel);
        ...
        less_than = 1'b0;
        #10;
        assert(pc_sel == 2'b01) else $error("BGE Taken: pc_sel = %b (Expected: 01)", pc_sel);
        $display("BGE Taken:     pc_sel = %b (Expected: 01)", pc_sel);

        // BGE Not Taken (less_than = 1) -> Expect pc_sel = 2'b00
        less_than = 1'b1;
        #10;
        assert(pc_sel == 2'b00) else $error("BGE Not Taken: pc_sel = %b (Expected: 00)", pc_sel);
